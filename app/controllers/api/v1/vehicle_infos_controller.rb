class Api::V1::VehicleInfosController < ApplicationController
    ...
    def get_models
        ...
        sql = "select model from vehicle_infos where year="+year+" and make='"+make+"' order by model asc;"
        records_array = ActiveRecord::Base.connection.execute(sql).values
        render :json => records_array
    end
    ...
    def get_body_styles
        ...
        sql = "select unnest(body_styles) from vehicle_infos where year="+year+" and make='"+make+"' and model='"+model+"'"
        records_array = ActiveRecord::Base.connection.execute(sql).values
        render :json => records_array
    end
